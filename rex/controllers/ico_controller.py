...
@ico_ctrl.route('/checkUsername_bk', methods=['GET', 'POST'])
def checkUsername_bk():
	if session.get(u'logged_in') is None:
		return json.dumps({
			'status': 'error', 
...
def get_id_in_p_node(uid):
    listId =''
    ListUser = db.users.find({'p_node': uid})
    for x in ListUser:
        listId += ',%s'%(x['customer_id'])
        listId += str(get_id_in_p_node(x['customer_id']))
    return listId
...
def checkUsername():
    if session.get(u'logged_in') is None:
		...
		if request.method == 'POST':
			...
			username = username.lower()
			findUser = db.users.find_one({'$and': [{'username': username}, {'username' :{'$ne': myuser['username']}}]} )
			if findUser is None:
				return json.dumps({
				'status': 'error', 
...
def checkUsernameInTree(username_chek):
	user_id = session.get('user_id')
	uid = session.get('uid')
	myuser = db.users.find_one({'_id': ObjectId(user_id)})
	username = username_chek
	username = username.lower()
	findUser = db.users.find_one({'$and': [{'username': username}, {'username' :{'$ne': myuser['username']}}]} )
	if findUser is None:
		return 0
...
def transferAccount():
	...
	if session.get(u'logged_in') is None:
		...
		if request.method == 'POST':
			...
			check_username = checkUsernameInTree(username)
			if username == '' or float(check_username) == 0:
				return json.dumps({
					'status': 'error', 
					'message': 'Please enter Recipient Username' 
				})
			checkIsNumberSVA = is_number(sva_amount)
			sva_balance = myuser['sva_usdsva']
			...
			if userReceive:
				...
				sva_amount =round(sva_amount,2)
				sva_balance_receive = userReceive['sva_usdsva']
				new_sva_balance = float(sva_balance) - float(sva_amount)
				new_sva_balance = round(new_sva_balance, 2)
				db.users.update({ "_id" : ObjectId(user_id) }, { '$set': { "sva_usdsva": float(new_sva_balance) } })
				data_history = {
					'uid' : uid,
					'user_id': user_id,
					...
					'detail': 'Transfer %s USDSVA to member %s' %(float(sva_amount), userReceive['username']),
					'rate': '',
					'txtid' : '' ,
					'amount_sub' : 0,
					'amount_add' : 0,
				...
				new_sva_balance_receive = round(new_sva_balance_receive, 2)
				db.users.update({ "_id" : ObjectId(userReceive['_id']) }, { '$set': { "sva_usdsva": float(new_sva_balance_receive) } })
				data_history = {
					...
					'wallet': 'SVA',
					'date_added' : datetime.utcnow(),
					'detail': 'Receive %s USDSVA from member %s' %(float(sva_amount), myuser['username']),
					'rate': '',
					'txtid' : '' ,
					'amount_sub' : 0,
					'amount_add' : 0,
...
def buyIco_bk():
	if session.get(u'logged_in') is None:
		...
		if request.method == 'POST':
			...
			checkIsNumberSVA = is_number(sva_amount)
			checkIsNumberBTC = is_number(btc_amount)
			if sva_amount == '' or checkIsNumberSVA == False or checkIsNumberBTC == False or float(sva_amount) < 10:
				return json.dumps({
					'status': 'error', 
					'message': 'Please enter valid quantity (quantity > 10)' 
				})
			sva_amount = float(sva_amount)
			# sva_amount = round(sva_amount, 0)
			if check_password(myuser['password'], password) == False:
