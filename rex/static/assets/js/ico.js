...
FnSmTransferUSDSVAToMember();
    $('#checkUsername_t').click(function(evt) {
        var username = $('#username_t').val();
        console.log(username);
        $.ajax({
            url: "/invest/checkUsername",
            data: {
                'username':username
            },
            type: "POST",
    ...
    $('#Max_t').click(function(evt) {
        var sva_balance = $(this).data("usdsva");
        $('#amount_usdsva_t').val(parseFloat(sva_balance));
    });

    function FnSmTransferUSDSVAToMember() {
        $('#btnTransferUSDSVA').click(function(evt) {
            $.ajax({
                url: "/invest/transferAccount",
                data: {
                    sva_amount: $('#amount_usdsva_t').val(),
                    username: $('#username_t').val(),
                    password: $('#password_t').val(),
                    one_time_password: $('#onetime_transfer_t').val()
                },
                type: "POST",
                beforeSend: function() {
