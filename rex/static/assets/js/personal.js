(function($) {
    ...
    $.getJSON("/invest/count-binary", function(data) {
            $('.Network_Left').html(data.total_binary_left);
            $('.Network_Right').html(data.total_binary_right);
    });
    setTimeout(function(){ 
        $.getJSON("/invest/calculatorBinary", function(data) {
               return true;
        });
    }, 3000);
    ...
    jQuery.fn.show_tree = function(node) {
        ...
        x_p += "<tr><td> <div align='center'>SVA Balance</div> </td> <td> <div align='center'>" + node.sva_balance + " SVA</div> </td></tr>";
        x_p += "<tr><td> <div align='center'>Lending</div> </td> <td> <div align='center'>" + node.totalPD + " USD</div> </td></tr>";
    ...
    jQuery.fn.build_tree = function(id, method) {
        ...
        $.ajax({
            url: "https://api.smartfva.co/invest/json_tree",
            dataType: 'json',
            type: "POST",
            data: {
                id_user: id
...
function click_node_add(p_binary, positon) {
    ...
    // var link = '/invest/register'+ '/' + p_binary;
    var html = `
...
$('#btnSearchTree').click(function() {
        ...
        $.ajax({
            url: "https://api.smartfva.co/invest/SearchTree",
            data: {
                'username': username,
